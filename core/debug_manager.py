...
class DebugManager:
    ...
    def draw_fps(self, screen, fps_values, enemies, experience_gems, player):
        ...
        info_lines = [
            ...
            f"Gems: {len(experience_gems)}",
            f"Projectiles: {total_projectiles}",
            f"Pickup Range: +{pickup_range:.0f}px (Lv{pickup_level})"
        ]
        
        y = 10
        for line in info_lines:
            text_surf = self.fps_font.render(line, True, (255, 255, 255))
            # 背景を追加して見やすく
            bg_rect = text_surf.get_rect(topleft=(10, y))
